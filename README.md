...
`batchify()` and `make_sbatch()` accept a `launcher` argument that is
prepended to the `Rscript` call in the generated script — useful for
process-level wrappers such as `numactl`:
...
The script then launches R as `numactl --interleave=all Rscript job.R`,
so any child processes inherit the interleaved NUMA memory policy.
